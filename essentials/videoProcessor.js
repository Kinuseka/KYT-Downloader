...
async function VideoProcess(VideoID, itag, meta, audio){
    ...
    if (jobstate == "active") {
      ...
      let job = await CombineQueue.addProcess(Ident=FinalID, JobType="Video", vsize=meta.contentLength, asize=audio.contentLength, VideoID, itag, audio.highestPossible, fileDir, filemp3, fileDone);
...
async function AudioProcess(VideoID, audio) {
    ...
    let jobstate = await job?.getState() || null;
    if (jobstate == "active") {
      console.log('[KYT-Processor] Existing job found but still active', FinalID);
      let state = {assumedProgress: job.progress, jobQueue: jobstate};
      return {result: 2, status: state, filemp3: filemp3}
    } else if (jobstate == "failed") {
