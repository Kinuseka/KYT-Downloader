...
router.post("/fetch_token", expressSession, async (req,res)=>{
    ...
    await (async (cleaned)=>{if (cleaned){console.log(`[KYT Downloader] Cleaned ${cleaned.toFixed(2)}MB from cache`)}})(await cleanCache(parseInt(selectedContent.contentLength), path.resolve(dir), ignore=[itag+videoID, itag+videoID+"_audified"]));
